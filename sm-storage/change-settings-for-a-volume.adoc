...
** World-Wide identifier (WWID) : un identifiant hexadécimal unique pour le volume.
** Identifiant unique étendu (EUI) -- un identifiant EUI-64 pour le volume.
** Identifiant de sous-système (SSID) -- l'identifiant de sous-système de la matrice de stockage d'un volume.
...
Paramètres de qualité de service
 a| 
*Désactiver définitivement Data assurance* -- ce paramètre n'apparaît que si le volume est Data assurance (DA) activé. DA vérifie et corrige les erreurs susceptibles de se produire lors de la communication des données entre l'hôte et la baie de stockage. Utilisez cette option pour désactiver définitivement DA sur le volume sélectionné. Lorsque cette option est désactivée, DA ne peut pas être réactivé sur ce volume.
...
Sélectionnez *menu:Accueil[opérations de visualisation en cours]* pour afficher la progression des opérations de modification en cours d'exécution pour le volume sélectionné.
